              ...
              {viewingSrcDetail && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10, maxHeight: '70vh', overflowY: 'auto' }} className="custom-scrollbar">
                  ...
                  {viewingSrcDetail.erotic && <div style={{ padding: '12px 16px', borderRadius: 12, background: 'rgba(220,38,38,0.03)', border: '1px solid rgba(220,38,38,0.12)', marginTop: 4 }}>
                    <label style={{ fontSize: 11, fontWeight: 700, color: '#dc2626' }}>🔞 涩涩分析</label>
                    {viewingSrcDetail.erotic.characterRoles?.length > 0 && <div style={{ marginTop: 6 }}><div style={{ fontSize: 10, fontWeight: 600, color: '#9b8e84', marginBottom: 2 }}>涩涩角色</div><div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>{viewingSrcDetail.erotic.characterRoles.map((cr: any) => <span key={cr.name} style={{ padding: '4px 10px', borderRadius: 8, background: 'rgba(220,38,38,0.06)', fontSize: 10, color: '#dc2626' }}>{cr.name}({cr.domSub}/{cr.bodyState}) 性癖:{cr.kinks?.join(',')||'无'} 羞耻:{cr.shameLevel}</span>)}</div></div>}
