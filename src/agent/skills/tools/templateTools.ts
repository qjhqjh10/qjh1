...
// Self-contained for skill system. Uses styleTemplateService and templateService
// from @/services/fileService for style and scene template creation.

import type { ToolDefinition, ToolResult, ToolExecutionContext } from '../types'

// Re-import types from shared type modules (these are not old agent types)
...
export const templateTools: ToolDefinition[] = [
      ...
      parameters: {
        ...
        properties: {
          ...
          dimensions: {
            type: 'object',
            description:
              '各维度分析。格式: {"维度key":{"description":"100-300字分析","examples":["原文例句"...],"writingRules":["规则"...],"vocabularyList":["词"...]}}。vocabularyList和writingRules必须在每个维度内部，不要放在顶层！',
          },
        },
      ...
      try {
        ...
        if (typeof dims === 'string') {
          try {
            ...
            return { status: 'error', summary: 'dimensions JSON 格式错误' }
          }
        }
        ...
        for (const [key, val] of Object.entries(dims as Record<string, any>)) {
          if (!val || typeof val !== 'object') {
            return {
              status: 'error',
              summary: `维度 ${key} 的值必须是对象{description,examples,writingRules,vocabularyList}`,
            }
          }
        }
        ...
        return {
          status: 'success',
          summary: `已创建风格模板: ${saved.name || tmpl.name}`,
          detail: `模板ID: ${saved.id}`,
        }
      } catch (e) {
        return {
          status: 'error',
          summary: `创建风格模板失败: ${e instanceof Error ? e.message : '未知错误'}`,
        }
      }
    },
  },

  {
    schema: {
      ...
      description:
        '创建场景模板并保存到场景工坊。根据细纲或上传文件分析创建。能推断的字段直接填值，无法确定的字段名列入 autoFields。先 read_file 读细纲JSON了解需求。',
      parameters: {
        type: 'object',
        properties: {
          name: { type: 'string', description: '模板名称' },
          type: {
            type: 'string',
            description:
              '小说类型。有效值: 情色小说|奇幻|都市小说|修仙小说|武侠小说|恋爱小说|古风小说|悬疑小说|历史小说|科幻小说|玄幻小说|灵异小说|轻小说|普通小说|穿越小说|末世小说|游戏小说',
          },
          // Universal scene fields
          ...
          characters: {
            ...
            description: '出场角色及情绪。每项: {characterId,characterName,emotion}',
          },
          ...
          autoFields: {
            ...
            items: { type: 'string' },
            description:
              '把握不好、无法确定的字段名列表。这些字段将显示AI自动按钮，用户可一键自动填充。不确定的字段优先列入此数组，不要强行填值。',
          },
          // Erotic scene fields (when type is 情色小说)
          intensity: { type: 'number', description: '情色浓度1-5' },
          selectedKinks: { type: 'array', items: { type: 'string' }, description: '玩法标签' },
          opening: {
            type: 'array',
            items: { type: 'string' },
            description: '开场阶段动作/心理描写要点',
          },
          climax: {
            type: 'array',
            items: { type: 'string' },
            description: '高潮阶段描写要点',
          },
          aftermath: {
            type: 'array',
            items: { type: 'string' },
            description: '余韵阶段描写要点',
          },
    ...
    executor: async (args: Record<string, unknown>): Promise<ToolResult> => {
      try {
        ...
        const str = (v: unknown, d = ''): string =>
          typeof v === 'string' ? v : v ? String(v) : d
        ...
        const config: Record<string, unknown> = {
          ...
          characters: (() => {
            ...
            if (Array.isArray(c))
              return c.map((x: any) =>
                typeof x === 'object'
                  ? x
                  : { characterId: '', characterName: String(x), emotion: '' },
              )
            if (typeof c === 'string' && c.trim())
              return c
                ...
                .map((s) => {
                  const m = s.match(/^(.+?)[:：](.+)$/)
                  return m
                    ? { characterId: '', characterName: m[1], emotion: m[2] }
          ...
          autoFields: (() => {
            const f = arr(args.autoFields)
            if (f.length === 0) return {}
            const o: Record<string, boolean> = {}
            for (const x of f) o[x] = true
            return o
          })(),
          intensity: Number(args.intensity || (args as any).eroticIntensity || 0),
          ...
          consentDynamic: '',
          aftercareDetail: '',
          senses: arr((args as any).senses || ['视觉', '听觉', '触觉']),
          dialogueRatio: '',
          subtextLevel: '',
          sentenceStyle: '',
          paragraphDensity: '',
        ...
        return {
          status: 'success',
          summary: `已创建场景模板: ${name}`,
          detail: `模板ID: ${tmpl.id}`,
        }
      } catch (e) {
        return {
          status: 'error',
          summary: `创建场景模板失败: ${e instanceof Error ? e.message : '未知错误'}`,
        }
      }
    },
  },
