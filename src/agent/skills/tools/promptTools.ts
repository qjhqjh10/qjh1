// ── Prompt Tools (3 tools) ──
// Self-contained for skill system. Reads from Zustand useSettingsStore.
// Allows the Agent to list, toggle, and update prompt templates.
    ...
    executor: async (_args: Record<string, unknown>): Promise<ToolResult> => {
      try {
        ...
        const lines = prompts.map(
          (p) => `[${p.enabled ? '✓启用' : '  关闭'}] ${p.id} | ${p.title} | 类型:${p.type}`,
        )
        return {
          status: 'success',
          summary: `${prompts.length} 个提示词模板`,
          detail: lines.join('\n'),
        }
      } catch (e) {
        return {
          status: 'error',
          summary: `列出提示词失败: ${e instanceof Error ? e.message : '未知错误'}`,
        }
      }
    },
  },

  {
    ...
    executor: async (args: Record<string, unknown>): Promise<ToolResult> => {
      try {
        ...
        if (enable) {
          const sameType = prompts.filter(
            (p) => p.type === target.type && p.id !== pid && p.enabled,
          )
          ...
          return {
            status: 'success',
            summary: `已启用「${target.title}」${disabled ? `（自动关闭: ${disabled}）` : ''}`,
          }
        }
        ...
        return {
          status: 'error',
          summary: `切换提示词失败: ${e instanceof Error ? e.message : '未知错误'}`,
        }
      }
    },
  },
    ...
    executor: async (args: Record<string, unknown>): Promise<ToolResult> => {
      try {
        ...
        if (Object.keys(updates).length === 0)
          return { status: 'error', summary: '没有提供要修改的字段' }
        if (!store.prompts.find((p) => p.id === pid))
          return { status: 'error', summary: `未找到提示词: ${pid}` }
        ...
        return {
          status: 'error',
          summary: `更新提示词失败: ${e instanceof Error ? e.message : '未知错误'}`,
        }
      }
    },
  },
