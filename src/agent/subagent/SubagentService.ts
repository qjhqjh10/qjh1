...
import { SUBAGENT_ANALYZE_PROMPT, SUBAGENT_EDIT_PROMPT } from './SubagentPrompt'
...
export type SubagentRole = 'analyze' | 'edit'
...
export const SUBAGENT_TOOL_NAMES = new Set(['analyze_file', 'edit_file_task'])
...
export interface SubagentOptions {
  ...
  contextWindow?: number   // 默认 64_000
  temperature?: number
...
export async function runSubagent(opts: SubagentOptions): Promise<SubagentResult> {
  ...
  const contextWindow = opts.contextWindow ?? 64_000

  // 温度配置（同 chatBridgeFactory：创作温度 + 工具执行温度上限）
