...
import { DocumentTextIcon, MagnifyingGlassIcon, FolderOpenIcon, PencilSquareIcon, PlusIcon, TrashIcon, TagIcon, SparklesIcon, PhotoIcon, GlobeAltIcon, WrenchScrewdriverIcon, XMarkIcon, CheckCircleIcon, ClockIcon, FireIcon, ChatBubbleLeftRightIcon, ClipboardDocumentCheckIcon } from '@heroicons/react/24/outline'

// v14.3.1: +子代理工具图标/标签（analyze_file/edit_file_task/verify_task/subagent_ask）
const ICONS: Record<string,any>={read_file:DocumentTextIcon,search_content:MagnifyingGlassIcon,list_directory:FolderOpenIcon,edit_file:PencilSquareIcon,create_file:PlusIcon,delete_file:TrashIcon,rename_file:TagIcon,analyze_text_style:SparklesIcon,search_images:PhotoIcon,kb_search:MagnifyingGlassIcon,kb_analyze:MagnifyingGlassIcon,http_get:GlobeAltIcon,browser_search:GlobeAltIcon,list_rules:WrenchScrewdriverIcon,analyze_file:DocumentTextIcon,edit_file_task:PencilSquareIcon,verify_task:ClipboardDocumentCheckIcon,subagent_ask:ChatBubbleLeftRightIcon,editor_rewrite:PencilSquareIcon}
const LABELS: Record<string,string>={read_file:'读取文件',search_content:'搜索内容',list_directory:'列出目录',edit_file:'编辑文件',create_file:'创建文件',delete_file:'删除文件',rename_file:'重命名',analyze_text_style:'风格分析',search_images:'搜索图片',kb_index_file:'知识库索引',kb_append_file:'知识库追加',list_prompts:'提示词列表',toggle_prompt:'切换提示词',update_prompt:'更新提示词',list_rules:'列出规则',http_get:'HTTP请求',http_fetch:'HTTP获取',browser_open:'打开浏览器',browser_search:'浏览器搜索',create_project:'创建项目',delete_project:'删除项目',kb_search:'搜索知识库',search_notes:'搜索笔记',tool_search:'搜索工具',find_files:'查找文件',batch_replace:'批量替换',analyze_file:'子代理分析',edit_file_task:'子代理修改',verify_task:'子代理验收',subagent_ask:'子代理追问'}

// v16.3.0(审计 M5/L2 修复): 步骤字段改为可选（与 Message.toolCallSteps 形状兼容——
// 原类型要求 durationMs/iteration 必填，实际数据源（消息持久化）不含，靠 as any 掩盖）；
// totalIterations 死 prop 删除；ICONS 补 kb_analyze/editor_rewrite，删 kb_list 死条目
type Step = { tool: string; status: string; summary?: string; durationMs?: number; iteration?: number }
interface Props { toolsUsed: string[]; toolCallSteps?: Step[]; breakdown?: { label: string; chars: number }[]; outputBreakdown?: { label: string; tokens: number }[]; iterationCount?: number; usage?: { prompt_tokens: number; completion_tokens: number; total_tokens: number }; onClose: () => void }
...
export function ToolDetailPanel(p: Props) {
  const { toolsUsed, toolCallSteps, breakdown, outputBreakdown, iterationCount, usage, onClose } = p
              ...
              {Array.from(groups.entries()).map(([iter,ss])=><div key={iter}>
                ...
                {ss.map((s,j)=>{const I=ICONS[s.tool]||WrenchScrewdriverIcon;const L=LABELS[s.tool]||s.tool;const e=s.status==='error'
                  return <div key={j} style={{display:'flex',alignItems:'flex-start',gap:6,padding:'2px 4px',borderRadius:4,opacity:e?.7:1}}>
                    ...
                    <div style={{flex:1,minWidth:0}}><span style={{fontWeight:600,color:e?'#dc2626':'#4a3f38'}}>{L}</span>{s.durationMs!==undefined&&<span style={{color:'#9b8e84',marginLeft:4}}>{s.durationMs}ms</span>}<div style={{color:e?'#dc2626':'#16a34a',wordBreak:'break-word'}}>{e?'❌':'✅'} {s.summary}</div></div>
                  </div>
                })}
              </div>)}
            </div>}
