...
const TABS: { key: Tab; label: string; icon: typeof DocumentTextIcon }[] = [
  { key: 'basic', label: '基础设定', icon: DocumentTextIcon },
...
export default function OutlinePage() {
  ...
  useEffect(() => {
    ...
    loadOutlineData<PowerSystem>(pp, 'power_system.json', { name: '', levels: [], description: '' }).then(d => {
      // Normalize old format (string[]) to new format (PowerLevel[])
        ...
        return (
          <div style={{ flex: 1, overflow: 'hidden', display: 'flex', justifyContent: 'center' }}>
            <div className="custom-scrollbar" style={{ width: '100%', height: '100%', overflowY: 'auto' }}>
              <RichTextEditor content={outlineContent} onContentChange={setOutlineContent} placeholder="在这里编写你的小说基础设定（故事大纲、核心剧情、主题思想）..." />
            </div>
          </div>
        )
      case 'worldbuilding':
        return (
          <div style={{ flex: 1, overflow: 'hidden', display: 'flex', justifyContent: 'center' }}>
            <div className="custom-scrollbar" style={{ width: '100%', height: '100%', overflowY: 'auto' }}>
              <RichTextEditor content={worldbuildingContent} onContentChange={setWorldbuildingContent} placeholder="在这里编写你的世界观设定..." />
            </div>
          </div>
        )
      case 'characters':
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {emotionData.segments.map((seg, i) => (
                <div key={i} style={{ ...cardStyle, display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <button onClick={() => saveEmotion({ segments: emotionData.segments.filter((_, j) => j !== i) })} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#d4ccc4' }}><TrashIcon style={{ width: 12, height: 12 }} /></button>
                </div>
              ))}
  ...
  const TAB_LABELS: Record<Tab, string> = {
    basic: '基础设定', worldbuilding: '世界观设定', characters: '角色档案',
    ...
    <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      ...
      {editingEmotion && (
        <EntityEditModal
          ...
          onSave={async () => { if (!editingEmotion.dominantEmotion.trim()) return; const exists = emotionData.segments.some((s, i) => s.chapterStart === editingEmotion.chapterStart && s.chapterEnd === editingEmotion.chapterEnd); const next = exists ? emotionData.segments.map(s => s.chapterStart === editingEmotion.chapterStart && s.chapterEnd === editingEmotion.chapterEnd ? editingEmotion : s) : [...emotionData.segments, editingEmotion]; await saveEmotion({ segments: next }); setEditingEmotion(null) }}
          width={380}
        >
          <div style={{ display: 'flex', gap: 10 }}>
