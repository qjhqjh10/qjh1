...
//
// ⚠️ 双通道迁移（2026-08-11 审计结论，刻意分开，不要合并）：
//   主进程通道 = 本文件（App 启动时对 electron-store 权威源执行，见 App.tsx 启动逻辑）；
//   渲染层通道 = src/store/index.ts 的 migrateSettings（persist rehydrate 时对
//     localStorage 镜像执行）。两进程各自启动时对各自存储执行，无法共享同一函数实例，
//     且镜像层看不到主进程的迁移结果。
//   修改约定：字段级迁移逻辑【两处必须同步修改】——尤其 v16.2.0 的 image*→secondary*
//     搬移（本文件 :66-93 与 store migrateSettings v10 分支）。改一处不改另一处 → 新旧
//     配置迁移行为不一致（主进程权威源已迁、镜像层未迁 → 界面显示旧配置）。新增字段
//     迁移时请同时在两处实现，并在此注释互相引用。
